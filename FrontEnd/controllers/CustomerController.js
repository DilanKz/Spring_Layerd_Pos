...
let baseURI='http://localhost:8080/Spring_Database_war/';
...
function addCustomersToTable() {
    ...
    $.ajax({
        ...
        success:function (res) {
            ...
            incrementCustomerID(data[data.length-1].id);
            cusIDF.val(newCusID);

            for (let i = 0; i < data.length; i++) {
                let row = $('<tr> <td>'+ data[i].id +'</td> <td>'+ data[i].name +'</td> <td>'+ data[i].address +'</td> <td>'+ data[i].salary +'</td> </tr>');
...
btnDelete.click(function (){
    ...
    if (confirm("Are you sure you want to delete this Customer?")) {
        ...
        $.ajax({
            url:baseURI+'customer?cusID='+cusID,
            method: "delete",
            success:function (res) {
                addCustomersToTable();
                clearCustomerFields();
...
cusContactF.keyup(function (event){
    ...
    if (itemPriceRegex.test(cusContactF.val())){
