...
function addItemsToTable() {
    $.ajax({
        ...
        success:function (res) {
            ...
            tblItems.empty();
            console.log()
            incrementItem(data[data.length-1].code);
            itemIdF.val(newItemID);

            for (let i = 0; i < data.length; i++) {
                let row = $('<tr> <td>'+ data[i].code +'</td> <td>'+ data[i].description +'</td> <td>'+ data[i].qtyOnHand +'</td> <td>'+ data[i].unitPrice +'</td> </tr>');
                tblItems.append(row);
            }
